...
var url = "http://localhost:3030/volvo/query";
var query = [
    "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>",
    "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>",
    "SELECT * WHERE {",
    "?sub rdfs:subClassOf* <http://www.w3.org/ns/ssn/System> .",
    "}"
].join(" ");
...
function httpGetAsync(theUrl) {
    ...
    }).responseText)['results']['bindings'];
}

var queryUrl = url + "?query=" + encodeURIComponent(query)
var res = httpGetAsync(queryUrl).map(x => x['sub']['value'].split('/').pop())
...
var query = [
    ...
    "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>",
    "SELECT * WHERE {",
    "?sub rdfs:subClassOf* <http://www.w3.org/ns/sosa/ObservableProperty> .",
    "}"
].join(" ");
...
var queryUrl = url + "?query=" + encodeURIComponent(query)
var props = httpGetAsync(queryUrl).map(x => x['sub']['value'].split('/').pop());
...
*/
    ...
    LGraphNode.prototype.serialize = function() {
        ...
        if (this.fault_table){
            o.fault_table = this.fault_table;
        }
...
*/
    ...
    function LGraphCanvas(canvas, graph, options) {
		...
		this.onConnectionChange =  null; //called before any link changes
		this.onBeforeChange = function(){
        var data = JSON.stringify(graph.serialize());
        console.log(data)
        LiteGraph.history.push(data)
        console.log(LiteGraph.history.length)
    ...
    LGraphCanvas.prototype.processKey = function(e) {
        ...
        if (e.type == "keydown") {
            ...
            if (e.keyCode == 90 && e.ctrlKey) {
              if(!LiteGraph.history.length)
          			return;

          		var step = LiteGraph.history.pop();
          		LiteGraph.post_history.push(JSON.stringify(this.graph.serialize()));
              this.graph.clear();
              this.graph.configure(JSON.parse(step));
            }

            if (e.keyCode == 89 && e.ctrlKey) {
              if(!LiteGraph.post_history.length)
          			return;

          		var step = LiteGraph.post_history.pop();
          		LiteGraph.history.push(JSON.stringify(this.graph.serialize()));
              this.graph.clear();
              this.graph.configure(JSON.parse(step));
	...
	*/
    ...
    $('.degree-selected').change(function () {
        ...
        $("#textform").remove();
        if (selectedItem === 'snowflake') {
            $('<div id="textform"><br><br><label>Database: </label><input type="text" name="hasDatabase" id="hasDatabase"><br><label>Table: </label><input type="text" name="hasTable" id="hasTable"><br><label>Column: </label><input type="text" name="hasColumn" id="hasColumn"><br><label>Schema: </label><input type="text" name="hasSchema" id="hasSchema"></div>').appendTo('#sourceform'); 
        }
        if (selectedItem === 'kafka') {
            $('<div id="textform"><br><br><label>Key: </label><input type="text" name="hasKey" id="hasKey"><br><label>Topic: </label><input type="text" name="hasTopic" id="hasTopic"></div>').appendTo('#sourceform'); 
        }
    });
    ...
    root.addService = function(node)
    {
        ...
        cell1.innerHTML = "<b>Properties</b>";
        table.appendChild(thead);
        var span = document.createElement("span")
        span.className = "table-add glyphicon glyphicon-plus"
        span.addEventListener('click', function(){
            ...
            $("#dialog").dialog({
                autoOpen: false, // Dialog will not open on page load
                modal: true,     // Creates a modal dialog
                buttons: {
                "Save": function() {
                    var form = document.getElementById("sourceform");
                    var selectElement = document.getElementById("source-select");
                    var selectProp = document.getElementById("property-select");
                    var selectedValue = selectElement.value;
                    var propertyValue = selectProp.value
                    
                    var formData = new FormData(form);
                    
                    // Convert form data to object
                    var formObject = {'source':selectedValue, 'property':propertyValue};
                    formData.forEach(function(value, key){
                        formObject[key] = value;
                    });
        
                    // Display form values (you can replace this with sending data to server)

                    var n_row = table.insertRow(-1);
                    var n_cell_type = n_row.insertCell(0);
                    formObject['row_nr'] = table.rows.length-1
                    node.sources[formObject['row_nr']] = formObject
                    n_cell_type.innerHTML = formObject['source'] + ' ' +formObject['property']
                    
                    $("#sourceform").trigger( "reset" );
                    $("#textform").remove();
                    $(this).dialog('close');
                    
                    n_cell_type.addEventListener('click', function(){
                        ...
                        $("#dialog").dialog({
                            ...
                            "Delete": function(){
                                delete node.sources[ind];
                                dct = {};
                                for (let key in node.sources) {
                                    if (key < ind){
                                        dct[key] = node.sources[key];
                                    }else{
                                        dct[key-1] = node.sources[key];
                                    }
                                }
                                node.sources = dct;
                                row.remove();
                        ...
                        console.log(node.sources)
                        console.log(node.sources[ind]["property"])
                        if (node.sources[ind]['source'] === 'snowflake') {
                            $("#source-select").val('snowflake')
                            $("#property-select").val(node.sources[ind]["property"]).change()
                            $('<div id="textform"><br><br><label>Table: </label><input type="text" name="hasTable" id="hasTable"><br><label>Column: </label><input type="text" name="hasColumn" id="hasColumn"><br><label>Schema: </label><input type="text" name="hasSchema" id="hasSchema"></div>').appendTo('#sourceform'); 
                            $("#hasTable").val(node.sources[ind]["hasTable"])
                            $("#hasColumn").val(node.sources[ind]["hasColumn"])
                            $("#hasSchema").val(node.sources[ind]["hasSchema"])
                            $("#hasDatabase").val(node.sources[ind]["hasDatabase"])
                        }
                        if (node.sources[ind]['source'] === 'kafka') {
                            $("#source-select").val('kafka')
                            $("#property-select").val(node.sources[ind]["property"]).change()
                            $('<div id="textform"><br><br><label>Key: </label><input type="text" name="hasKey" id="hasKey"><br><label>Topic: </label><input type="text" name="hasTopic" id="hasTopic"></div>').appendTo('#sourceform'); 
                            $("#hasTopic").val(node.sources[ind]["hasTopic"])
                            $("#hasKey").val(node.sources[ind]["hasKey"])
                        }
                        $("#dialog").dialog("open");
        ...
        if ("sources" in node){
            console.log(node.sources)
            var arrayLength = Object.keys(node.sources).length
            console.log(arrayLength)
            for (var i=0; i < arrayLength; i++){
                var n_row = tbody.insertRow(-1);
                var n_cell_type = n_row.insertCell(0);
                n_cell_type.innerHTML = node.sources[i+1]["source"] + ' ' +node.sources[i+1]["property"]
                n_cell_type.addEventListener('click', function(){
                    var row = this.parentNode
                    var ind = this.parentNode.rowIndex;
    ...
    root.addTable = function(node)
		{
            ...
            cell2.innerHTML = "<b>Cause</b>";
            var cell3 = row.insertCell(2);
            cell3.innerHTML = "<b>Mitigation</b>";
            ...
            span.addEventListener('click', function(){
                var n_row = table.insertRow(-1);
                var n_cell_fault = n_row.insertCell(0);
                n_cell_fault.innerHTML = 'Enter Fault'
                //n_cell_fault.contentEditable = "true";
                //n_cell_fault.addEventListener('input', function(){node.fault_table = tableToJson(table); console.log(node.fault_table)});
                n_cell_fault.addEventListener('click', function(){
                    n_cell_fault.clicked = true;
                    n_cell_fault.style.backgroundColor = "gray";
                    ...
                    $('#jstree_demo_div').jstree({
                      ...
                      $('#jstree_demo_div').bind("dblclick.jstree", function (event, data) {
                        if (n_cell_fault.clicked){
                            var elm = $(event.target).closest("a");
                            var data = $(elm[0]).text();
                            console.log(data)
                            n_cell_fault.innerHTML = JSON.parse(JSON.stringify(data))
                            node.fault_table = tableToJson(table)
                            console.log(node.fault_table)
                            $('#jstree_demo_div').hide();
                        }
                        n_cell_fault.style.backgroundColor = null;
                        n_cell_fault.clicked = false;
                     });
                     $('#jstree_demo_div').on('set_text.jstree', function (e, data) {
                            var objtree = $('#jstree_demo_div').jstree(true).get_json();
                            var fulltree = JSON.stringify(objtree); 
                            $.ajax({
                                type: "POST",
                                url: "/cgi-bin/save_faults.py",
                                data: { param: fulltree},
                            }).done(function( o ) {
                                console.log(o)
                            });
                ...
                var n_cell_cause = n_row.insertCell(1);
                n_cell_cause.innerHTML = 'Enter Cause'
                //n_cell_cause.contentEditable = "true";
                    //n_cell_cause.addEventListener('input', function(){node.fault_table = tableToJson(table); console.log(node.fault_table);s});
                
                    n_cell_cause.addEventListener('click', function(){
                        n_cell_cause.clicked = true;
                        n_cell_cause.style.backgroundColor = "gray";
                        $('#jstree_cause_div').show()
                        $('#jstree_cause_div').jstree({
                            "core" : {
                              "dblclick_toggle" : false,
                              "animation" : 0,
                              "check_callback" : true,
                          ...
                          $('#jstree_cause_div').bind("dblclick.jstree", function (event, data) {
                            if (n_cell_cause.clicked){
                                var elm = $(event.target).closest("a");
                                var data = $(elm[0]).text();
                                console.log(data)
                                n_cell_cause.innerHTML = JSON.parse(JSON.stringify(data))
                                node.fault_table = tableToJson(table)
                                console.log(node.fault_table)
                                $('#jstree_cause_div').hide()
                            }
                            n_cell_cause.style.backgroundColor = null;
                            n_cell_cause.clicked = false;
                         });
                         $('#jstree_cause_div').on('set_text.jstree', function (e, data) {
                                var objtree = $('#jstree_cause_div').jstree(true).get_json();
                                var fulltree = JSON.stringify(objtree); 
                                $.ajax({
                                    type: "POST",
                                    url: "/cgi-bin/save_causes.py",
                                    data: { param: fulltree},
                                }).done(function( o ) {
                                    console.log(o)
                                });
                         ...
                         $('#jstree_cause_div').on('delete_node.jstree', function (e, data) {
                            var objtree = $('#jstree_cause_div').jstree(true).get_json();
                            var fulltree = JSON.stringify(objtree); 
                            $.ajax({
                                type: "POST",
                ...
                var n_cell_mitigation = n_row.insertCell(2);
                n_cell_mitigation.innerHTML = 'Enter Mitigation'
                n_cell_mitigation.contentEditable = "true";
                n_cell_mitigation.addEventListener('input', function(){node.fault_table = tableToJson(table); console.log(node.fault_table);});

                var n_cell_delete = n_row.insertCell(3);
                var delete_span = document.createElement("div")
                delete_span.className = "glyphicon-minus"
                delete_span.onclick = function(element){
                    var row=element.target.parentNode.parentNode;
                    row.parentNode.removeChild(row);
                    node.fault_table = tableToJson(table)
                }
                n_cell_delete.appendChild(delete_span)
                node.fault_table = tableToJson(table);
            })
            elem.appendChild(span)
            

			if ("fault_table" in node){
                console.log("here")
                console.log(node.fault_table)

                var arrayLength = node.fault_table.length;
                for (var i=0; i < arrayLength; i++){
                    var n_row = table.insertRow(-1);
                    var n_cell_fault = n_row.insertCell(0);
                    n_cell_fault.innerHTML = node.fault_table[i]["fault"]
                    
                    //n_cell_fault.contentEditable = "true";
                    //n_cell_fault.addEventListener('input', function(){node.fault_table = tableToJson(table)});
                    n_cell_fault.addEventListener('click', function(){
                        n_cell_fault.style.backgroundColor = "gray";
                        n_cell_fault.clicked = true;
                        console.log("click");
                    ...
                    var n_cell_cause = n_row.insertCell(1);
                    n_cell_cause.innerHTML = node.fault_table[i]["cause"]
                    //n_cell_cause.contentEditable = "true";
                    //n_cell_cause.addEventListener('input', function(){node.fault_table = tableToJson(table); console.log(node.fault_table);s});
                ...
                n_cell_cause.addEventListener('click', function(){
                    ...
                    var n_cell_mitigation = n_row.insertCell(2);
                    n_cell_mitigation.innerHTML = node.fault_table[i]["mitigation"]
                    n_cell_mitigation.contentEditable = "true";
                    n_cell_mitigation.addEventListener('input', function(){node.fault_table = tableToJson(table)});
                ...
                //n_cell_fault.contentEditable = "true";
                //n_cell_fault.addEventListener('change', function(){node.fault_table = tableToJson(table)});
                n_cell_fault.addEventListener('click', function(){
                    n_cell_fault.clicked = true;
                    n_cell_fault.style.backgroundColor = "gray";
                    $('#jstree_demo_div').show()
    ...
    LGraphCanvas.prototype.showShowNodePanel = function( node )
	{
		...
		function inner_refresh()
		{
            ...
            panel.addWidget("string","Description", node.description, {}, fUpdate)
            panel.addWidget("combo","SubType", node.subtype, {values: LiteGraph.NODE_TYPES}, fUpdate);
    ...
    LGraphCanvas.onMenuSubType = function(value, options, e, menu, node) {
        console.log("here!!!")
        new LiteGraph.ContextMenu(
            LiteGraph.NODE_TYPES,
            { event: e, callback: inner_clicked, parentMenu: menu, node: node }
        );
        ...
        function inner_clicked(v) {
            ...
            var kV = Object.values(LiteGraph.NODE_TYPES).indexOf(v);
            node.subtype = kv
			var graphcanvas = LGraphCanvas.active_canvas;
        }
        ...
        if (node.getMenuOptions) {
            ...
            options = [
                ...
                },
                {
                    content: "SubType",
                    has_submenu: true,
                    callback: LGraphCanvas.onMenuSubType
                }];
