...
function Editor(container_id, options) {
    ...
    this.addToolsButton("cloud_button","Generate","imgs/icon-play.png", this.onCloudButton.bind(this), ".tools-left" );
            ...
            inputElement.addEventListener('change', function (data) {
                ...
                if (inputElement.files) {
                    ...
                    reader.addEventListener('loadend', function (load_data) {
                        if (reader.result){
                            if (hasExtension(file, ['.tcgraph'])){
                                graph.configure(JSON.parse(reader.result));
                            }else{
                                $('#spinner-div').show();
                                $.ajax({
                                    type: "POST",
                                    url: "/cgi-bin/plc_convert.py",
                                    data: { param: reader.result},
                                    complete: function () {
...
Editor.prototype.onCloudButton = function() {
            var data = JSON.stringify(graph.serialize());
            $.ajax({
                type: "POST",
                url: "/cgi-bin/semantify.py",
                data: { param: data},
            }).done(function( o ) {
                console.log(o)
                var file = new Blob([o]);
